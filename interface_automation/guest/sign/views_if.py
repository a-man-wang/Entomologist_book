...
def add_guest(request):
    ...
    if len(guest_list) >= event_limit:
        return JsonResponse({'status': 10023, 'message': 'event number is full'})
...
def get_guest_list(request):
    ...
    if eid != '' and phone != '':
        guest = {}
        try:
            result = Guest.objects.get(phone=phone,ebent_id=eid)
        except ObjectDoesNotExist:
            return JsonResponse({'status': 10022, 'message': 'query result is empty'})
        else:
            guest = {'realname': result.realname, 'phone': result.phone, 'email': result.email, 'sign': result.sign, }
